...
from Modules.progressbar import widgets as progressbar_widgets
from Modules.progressbar import progressbar as progressbar
import os
import sys
import json
import time
...
class ProgressBar(object):
    ...
    :param String type: Three types: "minute", "hour", "second"; corresponds displaying iters/minute iters/hour and iters/second.
        ...
        print("Time Elapsed: %.2fs, or %.2fmins, or %.2fhours" % ((self.EndTime-self.StartTime),
              (self.EndTime-self.StartTime)/60, (self.EndTime-self.StartTime)/3600))
...
def ExportToPkl(AbsolutePath, Content):
    ...
    try:
        ...
        logger.info("Pickle data of " + AbsolutePath +
...
def ExportToJsonNodeLinkData(AbsolutePath, GraphContent):
    ...
    try:
        ...
        logger.info("JsonNodeLinkData of " +
                    AbsolutePath+" written successfully.")
        f.close()
...
:rtype nxGraph
    ...
    try:
        ...
        logger.info("JsonNodeLinkData of " +
                    AbsolutePath+" loaded successfully.")
        f.close()
        return GraphContent
...
def CombineSparseMatricesRowWise(MainMatrix, AddedMatrix):
    ...
    if MainMatrix.size == 0:
        MainMatrix = scipy.sparse.csr_matrix(
            [np.zeros(AddedMatrix.shape[1], dtype=int)])
